...
- **Single-writer for coordination parts**: `[Content_Types].xml` and `word/_rels/document.xml.rels` have one writer each — the shared accumulators (`ContentTypes` for content types, body `PartRels` for doc rels) held by the body `DocxAssetRegistry` instance (named `bodyAssetRegistry` at call sites). Subsystems registering Overrides / Defaults / Relationships call `accumulator.ensureOverride(...)` / `accumulator.appendRel(...)`; `WritableArchive.set()` rejects direct writes to those two paths at runtime (`lib/xml/writable-archive.ts`). The earlier read-modify-write-via-`replacements.set` pattern silently lost entries when a later `flushTo` overwrote them — numbering scheme dropped on blank-source apply was the first reported manifestation. Ordering also matters: all accumulator-mutating subsystems must run before the registry's `flushTo`, or their additions are serialized out of the final archive.
- **Engine-managed paragraphs use `_`-prefixed styleIds**: scaffolding paragraphs the engine emits as siblings to user content (currently `_HiddenChapterCounter` next to each heading carrying the hidden chapter SEQ — keeps the heading paragraph's text content clean so STYLEREF / TOC / REF / nav-pane / copy-paste extraction doesn't pull in vanished counter text). `walkIndexedParagraphs` and `DocumentParser.handleParagraph` skip `_`-prefix styleIds so agent-facing 1-based paragraph indices reflect user-visible content order, not raw DOM order. `walkBodyParagraphs` deliberately does NOT skip — the caption / numbering counter simulator walks engine paragraphs to see their SEQ fields.
- **STYLEREF emit is locale-neutral or rejected**: Word translates the display names of built-in styles per UI language (`heading 1` ↔ `标题 1`, `Title` ↔ `标题`, `Caption` ↔ `题注`, etc.) and resolves STYLEREF field codes against the *localized* form, not the English canonical name stored in `<w:name>`. `emitInlineStyleRef` (`lib/edit/fields/inline-fields.ts`) drives a three-way dispatch on the resolved `StyleInfo`: outline-bound → `STYLEREF N` (1-9 numeric, locale-neutral); custom non-built-in name → `STYLEREF "<name>"` (custom names aren't translated); built-in localizable non-outline (Title / Subtitle / Caption / etc.) → throw with a clear migration hint (bind to an outline level, or rename to a custom name). The built-in name table lives at `lib/parse/builtin-styles.ts` and is shared with the style-collision preflight.
