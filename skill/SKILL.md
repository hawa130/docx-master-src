...
- "Installing a list-style for one occurrence isn't worth a second standardize pass" → A second pass takes seconds (`apply_styles` + a single-level `numbering`, or `migrate_numbering` after the heading pass). Output correctness > pass-count economy. If the content has lists, install the list-style.
