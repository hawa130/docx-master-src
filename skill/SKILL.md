...
- **Match content shape to slot shape.** List items default to body weight; bold only when the source explicitly emphasizes:
  ...
  - **Equation** (display math + optional right-aligned number) → `{ "type": "equation", "latex": "..." }` block for display math + a numbered paragraph **right-aligned on the same visual line** (standard 学术 convention — `equation centered ... (1)` with number flush right). Compose via a 3-column borderless `TableBlock`: empty left / equation middle / numbered paragraph right. The numbered paragraph carries the anchor for cross-refs; the equation paragraph itself holds no readable text. Inline math uses `{ "math": "..." }` inside Paragraph.text — see [references/equations.md](references/equations.md).
