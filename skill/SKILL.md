...
Mutates Word (.docx) OOXML directly: classify paragraph roles, inject named styles, migrate manual numbering, import template styles, audit conformance. Output is a new docx plus a change report; the original file is never touched.
...
|---|---|---|
| `standardize` | The user wants style / numbering / structural changes applied to a docx. Covers full-doc reshape, narrow targeted edits, and the manual-XML escape hatch. *Illustrative phrasings: "帮我排一下版", "套学校格式", "加个 X 样式 / 其他不动", "Heading2 字号改小一号".* | [references/standardize.md](references/standardize.md) |
...
When intent is genuinely ambiguous (mixed scope, broad request that could be standardize or audit), prefer asking one focused question over guessing.
