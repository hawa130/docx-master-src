...
| `migrate_captions` | `node scripts/migrate_captions.js <file> [--style <styleId>]` | Read-only detector for manually-numbered caption-shaped paragraphs (e.g. typed `图 2.1` with no SEQ field) — agent uses the output to write apply config that converts them to `CaptionBlock`. |
