...
- **Match content shape to slot shape.** List items default to body weight; bold only when the source explicitly emphasizes:
  ...
  - **Table** (table + caption) → `{ "type": "table", ... }` block with `alignment: "center"` per academic / formal convention + `TableCaption` bound to a single-level counter scheme, **placed above the table** (mirror of figures). Plan TableCaption upfront; cross-refs anchor on the caption, not the table itself. Simple data tables fit in two lines (`rows: [[...]]`); merged cells / custom borders / mixed-format cells / `headerStyle` / column widths / autofit-vs-fixed layout are layered as needed — see [references/tables.md](references/tables.md).
