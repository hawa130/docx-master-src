...
*Illustrative phrasings: "看看这份合不合学校规范", "对照这个标准检查一下". Same surface words can land in `standardize` instead when the user actually wants the issues fixed; ask one focused question if intent is genuinely ambiguous.*
...
2. **Gather facts.** `overview` for the global picture; `inspect_style`, `inspect_style_def`, `inspect_section`, `find_paragraphs` for specifics. Same inspect tools the other commands use — just no `apply_styles`.
...
3. **Compare and report.** Produce a structured violation list:
   ...
   If the user wants to fix the violations after seeing the audit, that becomes a separate `standardize` request — don't auto-fix without permission.
