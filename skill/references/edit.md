...
- **`merge`** — `{ ..., "keepPPr"?: "first" | "last" }`. Combine the targeted paragraphs (≥ 2) into one. All `<w:r>` runs of every paragraph are concatenated into the surviving paragraph in document order; the surviving paragraph's `<w:pPr>` is kept from the `keepPPr` end (default `"first"`). Non-run children of the survivor (bookmarks, proofErr) are preserved; non-run children of the removed paragraphs are discarded. Use for "this paragraph got split mid-sentence by a hard break" cleanups where a `replace` would lose run-level rPr. Refused under `trackChanges: true` (run in a separate apply without trackChanges). Also refused if any target paragraph contains tracked changes, a complex field, or an SDT control — accept/reject revisions and resolve fields first.
